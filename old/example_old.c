...
void draw(void) {
	...
	cls();
	locate(1, MAPSIZE + 1);
	colorPrint(WHITE, NOCOLOR, "Coins: %d\n", coins);
	colorPrint(WHITE, NOCOLOR, "Torch: %d\n", torch);
	colorPrint(WHITE, NOCOLOR, "Moves: %d\n", moves);
	colorPrint(WHITE, NOCOLOR, "Level: %d\n", level);
	locate(1, 1);
	...
	for (j = 0; j < MAPSIZE; j++) {
		for (i = 0; i < MAPSIZE; i++) {
			if (0); /* (i == x && j == y) printf("@"); */
				...
				colorPrint(WHITE, NOCOLOR, "o"); 
			} else if (lvl[i][j] & STAIRS_DOWN) {
				colorPrint(WHITE, NOCOLOR, "<");
			} else if (lvl[i][j] & TORCH) {
				colorPrint(WHITE, NOCOLOR, "f");
			}
		}
...
int main(void) {
	...
	colorPrint(WHITE, NOCOLOR, "Welcome! Use WASD to move, ESC to quit.\n");
	...
	for (;;) {
		if (kbhit()) {
			...
			if (k == 'a') {
				colorPrint(DARKGREY, NOCOLOR, "WEASEL DETECTED\n");
			}
			colorPrint(WHITE, NOCOLOR, "%c\n", k);
		}
	}
	...
	cls();
    colorPrint(GREY, NOCOLOR, "You have been eaten by a grue.\n");
	colorPrint(WHITE, NOCOLOR, "Coins: %d\n", coins);
	anykey(NULL);
	resetColor();
	showcursor();
